...
const url = process.argv[2] || `http://${config.app.host}:${config.app.port}/${config.healthCheck.path}`;
if (options.log === true) {
  logger.info(`GET call for healthcheck at: ${url} ...`);
}
...
const request = http.get(url, (res) => {
  if (options.log === true) {
    ...
    if (res.statusMessage) {
      logger.info(`statusMessage: '${res.statusMessage}'`);
    }
    logger.info('----------------');
  }
